# POST INSTAGRAM — AZÚCAR OCULTA
**Fecha sugerida:** Mañana AM (7:30–9:00)
**Formato:** Carrusel 9 slides (proporción 4:5, 1080×1350)
**Keyword ManyChat:** `AZÚCAR`
**Landing destino:** `/azucar` → CTA a `reboot30.html`

---

## 🎯 ESTRATEGIA DEL POST

Dar valor MASIVO en el carrusel (que la gente lo guarde y comparta), pero dejar un "curiosity gap" fuerte para que comenten la keyword. La landing entrega MUCHO MÁS: los 10 nombres disfrazados, dónde se esconden, 3 "saludables" trampa, test de etiqueta de 15 seg y 3 swaps reales. El CTA final es al programa Reboot 30 gratis.

---

## 📱 CAPTION (para pegar al subir)

> El adulto promedio se come hasta **35 cucharadas de azúcar al día** sin darse cuenta.
>
> La OMS recomienda máximo 6.
>
> ¿Cómo pasa? La industria le cambió el nombre. "Sin azúcar añadida" no significa sin azúcar — significa que le pusieron maltodextrina, jarabe de agave, dextrosa, o alguna de las otras 7 que te muestro aquí 👆
>
> **Guarda este post** para leer etiquetas esta semana. Te va a cambiar la relación con el super.
>
> ¿Quieres la lista completa de los 10 nombres disfrazados + la guía para leer cualquier etiqueta en 15 segundos + 3 swaps que usas desde mañana?
>
> 👉 **Comenta "AZÚCAR"** y te mando todo por DM. Gratis. Es el primero de una serie que estoy soltando antes del Programa Reboot 30 que arranca el 4 de mayo.
>
> #ReBootLifestyle #AzucarOculta #LeerEtiquetas #EnergíaEstable #HabitosSaludables #ComidaReal #SaludConsciente

---

## 🎨 SLIDES DEL CARRUSEL

### SLIDE 1 — HOOK (la que para el scroll)
**Texto grande:**
> Hoy vas a comer azúcar
> sin saberlo.

**Subtexto (pequeño, abajo):**
> Y no es tu culpa. Te la escondieron.

**Visual:** Fondo crema. Tipografía serif grande. Cuchara con azúcar desenfocada atrás.

---

### SLIDE 2 — EL DATO QUE DUELE
**Texto:**
> El adulto promedio:
> **35 cucharadas / día**
>
> Lo que recomienda la OMS:
> **6 cucharadas / día**

**Subtexto:**
> Casi 6x más. Todos los días.

---

### SLIDE 3 — EL TRUCO DE LA INDUSTRIA
**Texto:**
> "Sin azúcar añadida"
>
> NO significa sin azúcar.
> Significa que le cambiaron el nombre.

**Subtexto (abajo):**
> Te enseño los 3 más disfrazados 👉

---

### SLIDE 4 — DISFRAZADO #1
**Título:** JARABE DE AGAVE
**Texto:**
> Te lo venden como "natural".
> Tiene **85% de fructosa**.
> Peor que el azúcar de mesa para tu hígado.

**Dónde se esconde:**
> Yogures "fit", barras de granola, jugos "sin azúcar"

---

### SLIDE 5 — DISFRAZADO #2
**Título:** MALTODEXTRINA
**Texto:**
> Disparó tu glucosa más rápido que el azúcar puro.
> Índice glucémico: **110** (azúcar = 65).

**Dónde se esconde:**
> Proteínas, suplementos "fitness", cereales, salsas light

---

### SLIDE 6 — DISFRAZADO #3
**Título:** CONCENTRADO DE JUGO DE FRUTA
**Texto:**
> "Endulzado con fruta" = marketing.
> Es azúcar concentrada sin la fibra que la frena.

**Dónde se esconde:**
> Yogures de niños, cereales "saludables", jugos naturales de caja

---

### SLIDE 7 — LA REGLA QUE TE SALVA
**Texto (grande):**
> Si el ingrediente termina en **"-OSA"**,
> es azúcar.

**Lista:**
> Glucosa · Fructosa · Sacarosa · Dextrosa · Maltosa · Lactosa

**Footer:**
> Sin importar cómo lo llamen en la etiqueta de adelante.

---

### SLIDE 8 — CURIOSITY GAP
**Texto:**
> Hay **7 nombres más**
> que son igual de disfrazados.
>
> + los productos del super donde se esconden (spoiler: muchos "saludables").
>
> + 3 swaps que usas desde mañana.

---

### SLIDE 9 — CTA
**Texto (grande):**
> Comenta
> **"AZÚCAR"**
> aquí abajo 👇

**Subtexto:**
> Te mando GRATIS por DM:
> ✅ Los 10 nombres disfrazados (PDF)
> ✅ Guía: leer etiquetas en 15 seg
> ✅ 3 swaps para tu día
>
> Es el primer drop antes del **Programa Reboot 30** (arranca 4 de mayo, gratis).

**Logo:** @ariereboot

---

## 📊 MÉTRICAS A TRACKEAR

| Métrica | Meta slot 1 |
|---|---|
| Guardados | >5% del alcance |
| Compartidos | >2% del alcance |
| Comentarios con "AZÚCAR" | >150 |
| CTR DM → landing | >40% |
| CTR landing → form reboot30 | >12% |
| Registros al programa desde este funnel | >60 |

---

## 🔄 FOLLOW-UP STORIES (mismo día)

- **Story 1 (AM):** Captura del post con sticker "comenta AZÚCAR"
- **Story 2 (mediodía):** Foto genérica de una etiqueta del super (tapando la marca o en primer plano solo la lista de ingredientes) con las palabras disfrazadas marcadas con círculo
- **Story 3 (PM):** Screenshot anónimo de un DM agradeciendo + "ya van X comentarios"
- **Story 4 (noche):** Recordatorio del programa 4 de mayo con link en bio
